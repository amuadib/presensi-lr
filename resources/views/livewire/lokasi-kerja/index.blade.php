<div>
                ...
                <div class="rounded-t mb-0 px-4 py-3 border-0">
                    <div class="flex flex-wrap items-center">
                        <div class="relative w-full px-4 max-w-full flex-grow flex-1">
                            <h3 class="font-semibold text-gray-800 text-2xl">Lokasi</h3>
                        </div>
                            ...
                            @else
                                @foreach ($lokasi as $l)
                                    <tr>
                                        ...
                                        <td
                                            class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-no-wrap p-4">
                                            <button wire:click.prevent="edit({{ $l->id }})"
                                            ...
                                            <button type="button"
                                                wire:click.prevent="$emit('confirmDelete', {{ $l->id }})"
    ...
    @else
        @include('livewire.lokasi-kerja.form')
    @endif
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.livewire.on('getLocation', () => {
                getGeoLocation()
            });
        });
        ...
        function getGeoLocation() {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(fillCoord);
            } else {
                console.log("Geolocation is not supported by this browser.")
            }
        }

        function fillCoord(position) {
            @this.set('form.lat', position.coords.latitude);
            @this.set('form.lon', position.coords.longitude);
